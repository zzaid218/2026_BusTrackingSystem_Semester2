...
class DirectionsService:
    """Service to call Google Directions (or a simulator) and normalize results.

    Use `base_url` to point at a local simulator when testing, e.g.
    `DirectionsService(base_url="http://localhost:5001/maps/api")`.
    """

    def __init__(self, base_url: str = "https://maps.googleapis.com/maps/api"):
    ...
    async def get_directions(
        ...
        # Normalize to a simple shape useful for the frontend
        ...
        return {"status": data.get("status"), "routes": routes}
#
